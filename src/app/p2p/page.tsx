...
export default function P2PPage() {
  ...
  const paymentTokenLabel = (addr?: string) => {
    ...
    if (addr === ethers.constants.AddressZero) return 'خارج الشبكة';
  ...
  const handlePostAd = async () => {
    if (newAd.type === "sell") {
      ...
      try {
        ...
        let paymentTokenAddr = ethers.constants.AddressZero;
        if (newAd.paymentMethod === 'FIRE') paymentTokenAddr = FIRE_CONTRACT_ADDRESS;
        if (newAd.paymentMethod === 'Asia') paymentTokenAddr = ASIA_TOKEN_ADDRESS && ASIA_TOKEN_ADDRESS !== 'NA' ? ASIA_TOKEN_ADDRESS : ethers.constants.AddressZero;
        if (newAd.paymentMethod === 'ZainCash') paymentTokenAddr = ZAIN_TOKEN_ADDRESS && ZAIN_TOKEN_ADDRESS !== 'NA' ? ZAIN_TOKEN_ADDRESS : ethers.constants.AddressZero;
        if (newAd.paymentMethod === 'Master') paymentTokenAddr = MASTER_TOKEN_ADDRESS && MASTER_TOKEN_ADDRESS !== 'NA' ? MASTER_TOKEN_ADDRESS : ethers.constants.AddressZero;
      ...
      try {
        ...
        if (newAd.paymentMethod === 'ZainCash') paymentTokenAddr = ZAIN_TOKEN_ADDRESS && ZAIN_TOKEN_ADDRESS !== 'NA' ? ZAIN_TOKEN_ADDRESS : ethers.constants.AddressZero;
        if (newAd.paymentMethod === 'Master') paymentTokenAddr = MASTER_TOKEN_ADDRESS && MASTER_TOKEN_ADDRESS !== 'NA' ? MASTER_TOKEN_ADDRESS : ethers.constants.AddressZero;
        if (paymentTokenAddr === ethers.constants.AddressZero) { toast.error('Buy ads require an on-chain payment token'); setActionLoading(null); return; }
  ...
  const confirmPlaceOrder = async () => {
    ...
    if (ad.type === "sell") {
      try {
        ...
        if (ad.paymentToken && ad.paymentToken !== ethers.constants.AddressZero && ad.paymentToken !== "") {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <select value={paymentTokenFilter} onChange={(e) => setPaymentTokenFilter(e.target.value)} className="p-2 bg-white rounded border border-gray-200 text-sm text-black">
                ...
                <option value={ethers.constants.AddressZero}>خارج الشبكة</option>
              </select>
            </div>
          </div>
        </div>
